        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Attractions Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
            ...
            .navigationTitle("Roadside Attraction")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Attraction Name", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Notes", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Attraction(title: titleText.isEmpty ? "Attraction" : titleText, value2: value2Text, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }
